...
class imageMergerNode : public LifecycleNode
{
    ...
    private:
        ...
        void subRealsenseDepth(custom_msgs::msg::RealsenseDepthMatrix msg);
        void subRealsenseRGB(sensor_msgs::msg::Image msg);
        void subOptrisTemp(custom_msgs::msg::OptrisTemperatureMatrix msg);
        ...
        std::shared_ptr<rclcpp::Subscription<custom_msgs::msg::RealsenseDepthMatrix>> realsense_depth_sub_;
        std::shared_ptr<rclcpp::Subscription<sensor_msgs::msg::Image>> realsense_rgb_sub_;
        std::shared_ptr<rclcpp::Subscription<custom_msgs::msg::OptrisTemperatureMatrix>> optris_temp_sub_;
        ...
        // Image Stacks
        std::stack<std::vector<std::vector<std::vector<unsigned char>>>> q_realsense_RGB_;
        std::stack<std::vector<std::vector<float>>>  q_realsense_Depth_;
        std::stack<std::vector<std::vector<float>>>  q_thermal_float_;

        pthread_mutex_t thermal_mutex_= PTHREAD_MUTEX_INITIALIZER;
        pthread_mutex_t rgb_mutex_    = PTHREAD_MUTEX_INITIALIZER;
        pthread_mutex_t depth_mutex_  = PTHREAD_MUTEX_INITIALIZER;
        bool debug_mode_ = false;
        
        // Functions for clearing stacks
        void clearqRGB(std::stack<std::vector<std::vector<std::vector<unsigned char>>>>  &q);
        void clearqFloat(std::stack<std::vector<std::vector<float>>> &q);
