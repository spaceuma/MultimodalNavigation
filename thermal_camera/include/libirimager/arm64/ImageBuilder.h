 ...
 */
class __IRDIRECTSDK_API__ ImageBuilder
{
...
public:
   ...
   * @param[in] alignStride Align to image stride
   * @param[in] decimalPlaces Decimal places of image data
   */
  ...
  /**
   * @param[in] w width of palette bar
   * @param[in] h height of palette bar
   * @param[out] img palette bar image. Methode allocates a array with size of 3*w*h. Free pointer with delete[].
   */
...
private:
  ...
  //Conversion divisor
  float _thermalDecimalPlaceDivisor;

  //Conversion offset
  float _thermalOffset;
